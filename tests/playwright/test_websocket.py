...
def test_webmail_loads_in_browser(linbpq_web, page, capture_js_errors):
...
def test_webmail_signon_via_browser(linbpq_web, page, capture_js_errors):
    ...
    with capture_js_errors(page) as js_errors:
        ...
        if "Message List" in page.content():
            assert not js_errors, f"JS errors during signon: {js_errors}"
            return
