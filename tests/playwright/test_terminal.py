...
def test_terminal_page_has_input_and_output_iframes(
    linbpq_web, page, capture_js_errors
):
    ...
    with capture_js_errors(page) as errors:
        ...
        srcs = [
            iframes.nth(i).get_attribute("src")
    ...
    assert not errors, f"JS errors loading terminal: {errors}"


def test_terminal_output_iframe_loads(linbpq_web, page, capture_js_errors):
    ...
    with capture_js_errors(page) as errors:
        ...
        assert output_frame is not None, (
            f"OutputScreen frame not found.  Frames: "
            f"{[f.url for f in page.frames]}"
        )
        body_html = output_frame.content()
        assert "<body" in body_html.lower() or "<html" in body_html.lower()
    assert not errors, f"JS errors in terminal frames: {errors}"


def test_terminal_close_form_targets_termclose(
    linbpq_web, page, capture_js_errors
):
    ...
    assert not errors, f"JS errors on terminal page: {errors}"
