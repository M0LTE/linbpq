...
def test_30_bbs_pg_no_shell_injection(linbpq_pg, tmp_path):
    """Connect to BBS, select TESTPG, send a payload that
    contains shell metacharacters in the same line as
    ``PG TESTPG``.  Assert no side-effect file is created — a
    correctly-validating implementation either rejects the input
    or escapes it before passing to the shell.
    """
